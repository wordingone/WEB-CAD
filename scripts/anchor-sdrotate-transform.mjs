/**
 * anchor-sdrotate-transform.mjs
 *
 * 8-turn no-reload anchor for SdRotate agent dispatch.
 * Leo directive: assert SCENE EFFECT (quaternion change > epsilon), not just dispatch fired.
 *
 * Pass = all 8 turns dispatch successfully (status:"success") AND quaternion changes
 * Fail = any turn dispatches "error", echoes text without dispatch, or quaternion unchanged
 */
...
const EPSILON = 0.01; // min quaternion angular deviation (radians)

const TURNS = [
  { q: "rotate the box 45 degrees",            verb: "SdRotate", expectEffect: true },
  { q: "rotate it another 45 degrees",          verb: "SdRotate", expectEffect: true },
  { q: "rotate it 90 degrees counterclockwise", verb: "SdRotate", expectEffect: true },
  { q: "undo",                                  verb: "SdUndo",   expectEffect: true },
  { q: "rotate the box 30 degrees clockwise",   verb: "SdRotate", expectEffect: true },
  { q: "rotate it 60 degrees",                  verb: "SdRotate", expectEffect: true },
  { q: "rotate 45 degrees around the X axis",   verb: "SdRotate", expectEffect: true },
  { q: "undo the last rotation",                verb: "SdUndo",   expectEffect: true },
];
...
async function captureQuaternion(ws, uuid) {
  return evaluate(ws, `(() => {
    const obj = window.__viewer?.getScene?.()?.getObjectByProperty("uuid", ${JSON.stringify(uuid)});
    return obj ? [obj.quaternion.x, obj.quaternion.y, obj.quaternion.z, obj.quaternion.w] : null;
  })()`);
}

function quatDelta(a, b) {
  if (!a || !b) return null;
  // Angular distance between two unit quaternions: 2 * acos(|a·b|)
...
async function run() {
  ...
  for (let i = 0; i < TURNS.length; i++) {
    ...
    const quatBefore = await captureQuaternion(ws, boxUuid);
    ...
    const quatAfter = await captureQuaternion(ws, boxUuid);
    const delta = quatDelta(quatBefore, quatAfter);
    ...
    log(`[T${i + 1}] quat before: ${JSON.stringify(quatBefore)}`);
    log(`[T${i + 1}] quat after:  ${JSON.stringify(quatAfter)}`);
    log(`[T${i + 1}] delta: ${delta?.toFixed(4)}rad`);
    ...
    if (!effectOk) {
      fail(`T${i + 1} "${q}": quaternion delta ${delta?.toFixed(4)}rad <= epsilon ${EPSILON}rad — no scene effect`);
    }

    pass(`T${i + 1}: "${q}" → delta=${delta?.toFixed(4)}rad, ${verb} status:success`);
  ...
  for (const { turn, q, delta, status } of results) {
    pass(`Turn ${turn}: "${q.slice(0, 40)}" delta=${delta?.toFixed(4)}rad status:${status}`);
  }
  pass(`All 8 SdRotate turns passed with scene-effect assertions`);
  ws.close();
  process.exit(0);
}
