...
| `brep-join` | Join | click two objects | `opStartTool(brep-join)` | `SdJoin` | canonical BRep operands create both the joined source record and joined display mesh from BRep data before mesh-geometry fallback; open faces/surfaces with coincident boundary edges are welded into one shell; separated closed solids concatenate as separate shells |
...
| BRep join/rebuild/contour | Join now creates joined display geometry from canonical BRep operands and welds coincident boundary edges for open canonical BRep face shells, but does not do tolerance-heavy sewing for arbitrary nonmatching trims; rebuild converts face surfaces to NURBS form but does not reparameterize to a new fit count; contour stitches BRep face-plane intersections into canonical curves but is not a full arbitrary surface/solid intersection kernel. |
