...
`scripts/cdp.ts` is the default path for interacting with the canonical tab. Every action goes through the app's normal pointer/keyboard pipeline — identical to a real mouse/keyboard event.
